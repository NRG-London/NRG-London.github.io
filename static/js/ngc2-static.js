/* ================================================================
   ...
   the animated engine is ngc2-live.js, which builds keyed DOM.
   Both read their palettes, scales, formatters and geometry from
   ngc2-core.js so the two cannot drift apart.

   REQUIRES ngc2-core.js to be loaded first.

   Public API:
     NGC2Static.state            -> { tone, palette }
     NGC2Static.setTone(name)
     NGC2Static.setPalette(name)
     NGC2Static.register(id, spec)   spec = { type, data, opts }
     NGC2Static.renderAll()
     NGC2Static.theme()          -> resolved colours for current state
   ================================================================ */
(function (global) {
  'use strict';

  var C = global.NGC2Core;
  if (!C) throw new Error('ngc2-static.js requires ngc2-core.js to be loaded first');

  /* ---------- shared tokens & maths (see ngc2-core.js) ---------- */
  var INK = C.INK;
  ...
  function txt(x, y, s, cls, attrs) {
    attrs = attrs || {};
    attrs.x = x; attrs.y = y; attrs.class = 'ngc2-t' + (cls ? ' ' + cls : '');
  ...
  function vbar(data, opts) {
    ...
    // Same ladder as ngc2-live.js: full label, shortened, then rotated. Both
    ...
    sc.ticks.forEach(function (tk) {
      ...
      svg += txt(m.l + G.tickDx, yy + G.tickDy, fmtCompact(tk, opts.unit), 'ngc2-axis', { fill: th.sub, 'text-anchor': 'end' });
    });
    // baseline
    ...
    data.forEach(function (d, i) {
      ...
      if (opts.valueLabels !== false && (hasVal || isNote)) {
        ...
        svg += txt(cx, place.y, lab, 'ngc2-val', {
      ...
      svg += txt(cx, catY, fit.labels[i], 'ngc2-cat', {
    ...
    (opts.targets || []).forEach(function (tg) {
      ...
      if (tg.label)
        svg += txt(cxOf(tg.labelAt != null ? tg.labelAt : (tg.to != null ? tg.to : n - 1)),
          ty - 10, tg.label, 'ngc2-val', { fill: col, 'text-anchor': 'middle', 'font-weight': 600 });
    });
  ...
  function gbar(data, opts) {
    ...
    data.categories.forEach(function (cat, ci) {
      ...
      svg += txt(catX, catY, fit.labels[ci], 'ngc2-cat', {
        fill: th.sub,
        'text-anchor': fit.rotate ? 'end' : 'middle',
  ...
  function line(data, opts) {
    ...
    var th = theme();
    var G = GEOM.line;
    // A COPY of the geometry's margins, not the object itself: the right margin
    // is sized per chart from its end-labels, and writing through to GEOM would
    // leak one chart's longest series name into every chart drawn afterwards.
    var W = G.W, H = G.H, m = { t: G.m.t, r: G.m.r, b: G.m.b, l: G.m.l };
    var iw = W - m.l - m.r, ih = H - m.t - m.b;
    var allV = [];
    data.series.forEach(function (s) {
      s.values.forEach(function (v) { if (v != null) allV.push(v); });
    });
    if (data.band) {
      [data.band.lo, data.band.hi].forEach(function (edge) {
        (edge || []).forEach(function (v) { if (v != null) allV.push(v); });
      });
    }
    var lo = Math.min.apply(null, allV), hi = Math.max.apply(null, allV);
    var sc;
    if (opts.zero !== false) {
      sc = niceScale(Math.min(0, lo), hi, G.ticks);
    } else {
      // Match ngc2-live.js's layoutLine exactly: the same 12% of air and the
      // same tight-scale search, so the baked view and the live view put the
      // line in the same place. Diverge here and the handover visibly jumps.
      var pad = Math.max((hi - lo) * 0.06, 0.4);
      sc = C.tightScale(lo - pad, hi + pad);
    }

    // Right margin sized to the direct end-labels, as layoutLine does.
    m.r = C.endLabelMargin(data.series.map(function (s) { return s.name; }),
                           13.5, G.m.r * 0.5) + G.endDx;
    iw = W - m.l - m.r;

    var y = lin(sc.min, sc.max, m.t + ih, m.t);
    var n = data.x.length;
    var x = lin(0, Math.max(1, n - 1), m.l, m.l + iw);
    var svg = '';
    ...
    // Peer range band, behind everything. Drawn first so the lines sit on it.
    if (data.band && data.band.lo && data.band.hi) {
      var top = [], bottom = [];
      for (var bi = 0; bi < n; bi++) {
        if (data.band.lo[bi] == null || data.band.hi[bi] == null) continue;
        top.push('L' + x(bi).toFixed(1) + ' ' + y(data.band.hi[bi]).toFixed(1));
        bottom.unshift('L' + x(bi).toFixed(1) + ' ' + y(data.band.lo[bi]).toFixed(1));
      }
      if (top.length > 1) {
        svg += el('path', { d: ('M' + top.join(' ').slice(1) + ' ' + bottom.join(' ') + ' Z'),
                            fill: th.bandFill, stroke: 'none' });
      }
    }

    /* x labels
       `opts.labelAt` names the indices worth labelling - year boundaries, for a
       series of four-week periods. Thinning to every other point instead would
       land on an arbitrary period and give a reader nothing to orient by. With
       no labelAt the old every-other behaviour stands, so bar-era callers are
       unaffected. */
    var labelAt = opts.labelAt;
    if (labelAt) {
      // Thinned here rather than by the caller, so the baked SVG and the live
      // render drop the same labels.
      labelAt = C.thinLabels(labelAt, data.x, function (i) { return x(i); }, 13.5);
    }
    data.x.forEach(function (lab, i) {
      if (labelAt) {
        if (labelAt.indexOf(i) === -1) return;
      } else if (n > G.thinAbove && i % 2 !== 0 && i !== n - 1) {
        return;
      }
      svg += txt(x(i), m.t + ih + G.catDy, lab, 'ngc2-cat', { fill: th.sub, 'text-anchor': 'middle' });
    });

    /* The emphasised series takes the highlight hue and the heavier stroke;
       everything else is context in grey. Colour follows the series' ROLE, not
       its position in the array, so adding a reference line never repaints the
       one the reader selected. */
    data.series.forEach(function (s, si) {
      var focused = opts.focus ? s.name === opts.focus : si === 0;
      var col = focused ? th.lineFocus : th.lineReference;
      var pts = [];
      for (var i = 0; i < n; i++) {
        if (s.values[i] == null) continue;
        pts.push({ i: i, x: x(i), y: y(s.values[i]) });
      }
      if (!pts.length) return;

      // Split the provisional tail into its own dashed path so the reader can
      // see WHERE the figures stop being final.
      var cut = (focused && opts.provisionalFrom != null) ? opts.provisionalFrom : null;
      var solid = cut == null ? pts : pts.filter(function (p) { return p.i <= cut; });
      var dashed = cut == null ? [] : pts.filter(function (p) { return p.i >= cut; });
      var toD = function (list) {
        return list.map(function (p, k) {
          return (k ? 'L' : 'M') + p.x.toFixed(1) + ' ' + p.y.toFixed(1);
        }).join(' ');
      };
      var stroke = focused ? G.firstW : G.restW;
      if (solid.length > 1) {
        svg += el('path', { d: toD(solid), fill: 'none', stroke: col,
                            'stroke-width': stroke, 'stroke-linejoin': 'round',
                            'stroke-linecap': 'round' });
      }
      if (dashed.length > 1) {
        svg += el('path', { d: toD(dashed), fill: 'none', stroke: col,
                            'stroke-width': stroke, 'stroke-dasharray': '4 3',
                            'stroke-linejoin': 'round', 'stroke-linecap': 'round' });
      }

      var last = pts[pts.length - 1];
      var prov = cut != null && last.i >= cut;
      svg += el('circle', {
        cx: last.x, cy: last.y, r: G.dotR,
        fill: prov ? (th.dark ? DEEP : '#ffffff') : col,
        stroke: prov ? col : 'none', 'stroke-width': prov ? 2 : 0
      });
      svg += txt(last.x + G.endDx, last.y + G.endDy, s.name, 'ngc2-end',
  ...
  function ranking(data, opts) {
    ...
    if (th.grid) sc.ticks.forEach(function (tk) {
      ...
      svg += txt(x(tk), m.t + ih + G.axisDy, fmtCompact(tk, opts.unit), 'ngc2-axis', { fill: th.sub, 'text-anchor': 'middle' });
    });
    ...
    rows.forEach(function (d, i) {
      ...
      svg += txt(m.l + G.labelDx, ry + rowH / 2 + G.textDy, d.label, 'ngc2-cat', {
        fill: d.highlight ? th.text : th.sub, 'text-anchor': 'end',
        'font-weight': d.highlight ? 600 : 400
      });
      svg += txt(x(d.value) + G.valueDx, ry + rowH / 2 + G.textDy, fmtCompact(d.value, opts.unit), 'ngc2-val', {
  ...
  function tilemap(data, opts) {
    ...
    BOROUGHS.forEach(function (b) {
      ...
      svg += txt(cx + cell / 2, cy + cell / 2 + G.codeDy, code, 'ngc2-tile-code', { fill: fg, 'text-anchor': 'middle', 'font-weight': 700 });
      if (has) svg += txt(cx + cell / 2, cy + cell / 2 + G.valDy, fmtCompact(vals[code], opts.unit), 'ngc2-tile-val', { fill: fg, 'text-anchor': 'middle', opacity: 0.9 });
    });
  ...
  function sparkline(values, opts) {
    ...
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" width="' + W + '" height="' + H + '" class="ngc2-spark" preserveAspectRatio="xMidYMid meet">' + svg + '</svg>';
  }

  /* ---------- shared chrome ---------- */
  function wrapSVG(W, H, inner) {
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" width="100%" preserveAspectRatio="xMidYMid meet" class="ngc2-svg" role="img">' + inner + '</svg>';
  }
  function legend(items) {
    var th = theme();
    var s = '<div class="ngc2-legend">';
    items.forEach(function (it) {
      s += '<span class="ngc2-legend__item" style="color:' + th.sub + '">' +
        '<span class="ngc2-legend__swatch" style="background:' + it.color + '"></span>' + esc(it.name) + '</span>';
    });
    return s + '</div>';
  }
  function seqLegend(lo, hi, unit, th) {
    var stops = th.seq.map(function (c, i) { return c + ' ' + (i / (th.seq.length - 1) * 100) + '%'; }).join(', ');
    return '<div class="ngc2-seqlegend">' +
      '<span class="ngc2-seqlegend__lab" style="color:' + th.sub + '">' + fmtCompact(lo, unit) + '</span>' +
      '<span class="ngc2-seqlegend__bar" style="background:linear-gradient(90deg,' + stops + ')"></span>' +
      '<span class="ngc2-seqlegend__lab" style="color:' + th.sub + '">' + fmtCompact(hi, unit) + '</span>' +
      '</div>';
  }
  ...
  function renderOne(id) {
    ...
    if (!spec || !node) return;
    var fig = node.closest('.ngc2-chart');
